/**
 * Schema字段说明文档生成器
 * 用于解析backend/models/下的所有模型并生成JSON格式的字段说明
 */
const fs = require('fs');
...
// 输出文件路径
const OUTPUT_FILE = path.join(__dirname, 'schema_descriptions.json');
...
// 直接生成预定义模型的Schema描述
function generateSchemaDescriptions() {
  ...
  // 输出到JSON文件
  fs.writeFileSync(OUTPUT_FILE, JSON.stringify(schemaDescriptions, null, 2), 'utf-8');
  console.log(`Schema文档已生成: ${OUTPUT_FILE}`);
}
...
function main() {
  try {
    ...
    console.error('生成Schema文档时出错:', err);
    process.exit(1);
  }
}
