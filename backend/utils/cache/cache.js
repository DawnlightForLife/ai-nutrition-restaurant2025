 ...
 * ✅ 所属层级：utils（通用工具层）
 * ✅ 功能说明：
 *   - 提供统一的缓存接口，支持 Redis 与内存双模式缓存
 *   - 自动降级处理，优先使用 Redis，失败时回退至内存缓存
 *   - 支持 TTL（过期时间）、状态查询、清空、关闭等功能
 * ✅ 使用方式示例：
 *   const { get, set, initRedisCache } = require('./cache');
...
const initRedisCache = async () => {
  try {
    ...
    redisClient = redis.createClient({
      ...
      retry_strategy: options => {
        ...
        if (options.error && options.error.code === 'ECONNREFUSED') {
          console.error('[Cache] Redis错误: Redis连接被拒绝，可能未启动Redis服务');
          return new Error('Redis服务连接失败');
        }
        if (options.total_retry_time > 1000 * 60 * 30) {
          console.error('[Cache] Redis错误: Redis重试时间超过30分钟，放弃连接');
          return new Error('Redis重试超时');
        }
        if (options.attempt > 10) {
          console.error('[Cache] Redis错误: Redis已重试10次，放弃连接');
    ...
    console.error('[Cache] 初始化Redis缓存失败:', error);
    return false;
  }
};

/**
 * 获取缓存值，自动支持 Redis 和内存缓存
...
const get = async (key) => {
  ...
  if (redisClient) {
    try {
      ...
      console.error(`[Cache] 从Redis获取缓存失败 (${key}):`, error);
      // Redis失败，降级到内存缓存
    }
  }
...
const set = async (key, value, ttl = 0) => {
  ...
  if (redisClient) {
    try {
      ...
      console.error(`[Cache] 设置Redis缓存失败 (${key}):`, error);
      // Redis失败，降级到内存缓存
    }
  }
...
const del = async (key) => {
  ...
  if (redisClient) {
    try {
      ...
      console.error(`[Cache] 删除Redis缓存失败 (${key}):`, error);
      success = false;
    }
  }
...
const clear = async () => {
  ...
  if (redisClient) {
    try {
      ...
      console.error('[Cache] 清空Redis缓存失败:', error);
      success = false;
    }
  }
...
const close = async () => {
  ...
  return new Promise((resolve) => {
    redisClient.quit((err) => {
      if (err) {
        console.error('[Cache] 关闭Redis连接失败:', err);
        resolve(false);
      } else {
