...
const NutritionData = require('../../models/nutrition/nutritionDataModel');
...
// 随机生成健康数据
async function generateNutritionData(userId, nutritionProfileId = null) {
  // 随机性别
  ...
  // 保存健康数据
  const savedNutritionData = await nutritionData.save();
  console.log(`已创建健康数据: ${savedNutritionData._id}`);
  
  // 如果有关联的营养档案，更新档案的相关字段
  if (nutritionProfileId) {
    const relatedProfile = await NutritionProfile.findById(nutritionProfileId);
    if (relatedProfile) {
      relatedProfile.related_nutrition_data = relatedProfile.related_nutrition_data || [];
      relatedProfile.related_nutrition_data.push(savedNutritionData._id);
  ...
  // 自动生成健康建议
  await savedNutritionData.analyzeAndGenerateAdvice();
  console.log(`已为健康数据 ${savedNutritionData._id} 生成健康建议`);
  
  return savedNutritionData;
}
...
async function generateDataForAllUsers(count = 3) {
  try {
    ...
    for (const user of users) {
      ...
      // 查找用户的营养档案
      const profiles = await NutritionProfile.find({ userId: user._id });
      console.log(`找到 ${profiles.length} 个营养档案`);
      
      for (const profile of profiles) {
        console.log(`为营养档案 ${profile._id} 生成健康数据`);
        await generateNutritionData(user._id, profile._id);
      }
    }
  } catch (error) {
    console.error('生成健康数据时出错:', error);
    process.exit(1);
  }
}
