...
async function getAllShardNames() {
  ...
  const shardCollections = collections.filter(coll => {
    ...
    return (
      ...
      name.startsWith('healthdata_user_') ||
...
async function generateShardReport() {
  try {
    ...
    shardNames.forEach(name => {
      ...
      else if (name.startsWith('healthdata_user_')) shardTypes.healthdata.push(name);
    ...
    // 默认集合列表，用于比较和计算分片率
    const defaultCollections = ['users', 'healthdata', 'orders', 'merchants'];
